...
export function Hero() {
  ...
  return (
    <Reveal className="min-h-screen flex flex-col justify-center items-start mt-20 md:mt-0 mb-24 md:mb-32">
      <h1 className="text-display-xl text-on-surface mb-8 max-w-4xl tracking-tighter leading-tight">
        Building Ideas Into <br/>
        <span className="text-gradient-primary">Digital Reality</span>
      </h1>
      <div className="text-xl md:text-2xl text-on-surface-variant mb-8 h-8 md:h-10">
        I am a <span className="text-secondary-fixed-dim typewriter-cursor pl-2">{roleText}</span>
      </div>
      <p className="text-body-md text-on-surface-variant max-w-2xl mb-10 opacity-85 leading-relaxed">
      ...
      <div className="flex flex-wrap gap-4 md:gap-5">
        <MagneticButton href="#work" className="px-7 md:px-8 py-3 md:py-3.5 bg-gradient-to-r from-primary-container to-secondary-container text-on-primary text-label-mono font-bold rounded shadow-lg glow-primary motion-transition flex items-center gap-2 hover:shadow-[0_0_40px_rgba(165,231,255,0.3)]">
          View Projects <ArrowRight size={18} />
        </MagneticButton>
        <MagneticButton href="https://www.linkedin.com/in/shailesh-sethi-3aab58378/" target="_blank" className="px-7 md:px-8 py-3 md:py-3.5 bg-transparent border border-primary text-primary text-label-mono font-bold rounded hover:bg-primary/10 hover-glow-primary motion-transition flex items-center gap-2">
