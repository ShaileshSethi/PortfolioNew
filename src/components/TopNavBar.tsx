...
export function TopNavBar() {
  return (
    <nav className="bg-surface/50 backdrop-blur-xl text-primary fixed top-0 left-0 right-0 w-full z-50 border-b border-white/10 shadow-[0_2px_20px_rgba(0,0,0,0.1)] flex justify-between items-center px-3 sm:px-4 md:px-12 h-14 sm:h-16 md:h-20">
      <a href="#" className="flex items-center shrink-0 hover:opacity-80 transition-opacity focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded">
        <img src={logoSrc} alt="Shailesh Sethi Portfolio" className="h-8 sm:h-10 md:h-14 w-auto block" />
      </a>
      ...
      <button className="md:hidden text-primary p-1 min-h-10 min-w-10 flex items-center justify-center">
        <Menu size={24} />
      </button>
    </nav>
  );
